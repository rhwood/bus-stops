...
struct BusStops: ParsableCommand {
    ...
    func run() throws {
        ...
        if let mrs = Station(rawValue: station) {
            WMATA.Rail.StationInformation(key: key, station: mrs).request() { infoResult in
            switch infoResult {
            case .success(let info):
                print("Got station information for \(mrs.name), using lat: \(info.latitude) long: \(info.longitude)")
                let bus = WMATA.Bus.StopsSearch(key: key, location: WMATALocation(radius: distance, latitude: info.latitude, longitude: info.longitude))
                bus.request { stopsResult in
                    switch stopsResult {
